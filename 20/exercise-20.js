/*
  ...
  - Exercise from: https://2022.adventjs.dev/challenges/2022/20
...
export function howManyReindeers(reindeerTypes, gifts) {
  const reindeerTypesCopy = reindeerTypes.sort(
    (a, b) => b.weightCapacity - a.weightCapacity
  );

  const distributeReindeers = giftWeight => {
    const allowedReindeerTypes = reindeerTypesCopy.filter(
      ({ weightCapacity }) => weightCapacity < giftWeight
    );
    ...
    return allowedReindeerTypes.map(({ weightCapacity, type }) => {
      ...
      return { type, num: reindeersNumber };
    });
  };
