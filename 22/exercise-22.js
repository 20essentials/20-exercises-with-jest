/*
  The Lights in Sync
  - Exercise from: https://2022.adventjs.dev/challenges/2022/22
...
export function checkStepNumbers(systemNames, stepNumbers) {
  const newObject = {};
  const { length } = systemNames;
  for (let i = 0; i < length; i++) {
    const name = systemNames[i];
    if (newObject[name] === undefined) {
      newObject[name] = [];
    }
    newObject[name].push(stepNumbers[i]);
  }

  const newArray = Object.values(newObject);

  return newArray.every(([a, b = Number.MAX_SAFE_INTEGER]) => a < b);
}