/*
  ...
  - Exercise from: https://2022.adventjs.dev/challenges/2022/5
...
export function getMaxGifts(giftsCities, maxGifts, maxCities) {
  ...
  const generateCombinations = (start, currentSum, cityCount) => {
    if (cityCount > maxCities || currentSum > maxGifts) return;

    maxSum = Math.max(maxSum, currentSum);
    if (maxSum === maxGifts) return;

    for (let i = start; i < giftsCities.length; i++) {
      generateCombinations(i + 1, currentSum + giftsCities[i], cityCount + 1);
    }
  };
