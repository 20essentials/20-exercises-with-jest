/*
  ...
  - Exercise from: https://2022.adventjs.dev/challenges/2022/13
...
export function getFilesToBackup(lastBackup, changes) {
  const sortedArray = changes.sort((a, b) => a[0] - b[0]);
  const arrayIds = sortedArray.map(el => {
    if (el[1] > lastBackup) {
      return el[0];
    }
  });

  return [...new Set(arrayIds.filter(Boolean))];
}
